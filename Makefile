...
GPG_FILES = gpg-agent.conf gpg.conf
...
# Step 7
setup-gpg-config:
	@echo -e "\nSetting up GPG config..."
	@for file in $(GPG_FILES); do \
		[ ! -f ~/.gnupg/$$file ] && ln -s $$(pwd)/$$file ~/.gnupg/$$file && echo "File '$$file' created." || echo "Skipping '$$file' as it already exists.";  \
	done
	@echo -e "Done."
